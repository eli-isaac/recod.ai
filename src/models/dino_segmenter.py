...
class DinoDecoder(nn.Module):
    ...
    def __init__(self, in_channels: int = 768, out_channels: int = 4, dropout: float = 0.1):
        super().__init__()
        
        # Conv blocks for each upsampling stage
        self.up1 = self._block(in_channels, 384, dropout)  # 768 → 384
        self.up2 = self._block(384, 192, dropout)          # 384 → 192
        self.up3 = self._block(192, 96, dropout)           # 192 → 96
        self.up4 = self._block(96, 48, dropout)            # 96 → 48
